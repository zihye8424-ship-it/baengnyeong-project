...
export const metadata = {
  title: "대청도 삼각산 여행 가이드 | 백령도의 모든 정보",
  description: "정상석이 자리한 해발 343m 대청도의 대표 산행 명소. 볼거리, 여행 팁, 백과사전식 정보와 방문 후기를 확인하세요.",
};
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-gray-900">
      ...
      <section className="relative h-[62vh] min-h-[480px] w-full overflow-hidden">
        <img src="/images/samgaksan.png" alt="대청도 삼각산" className="h-full w-full object-cover" />
        ...
        <div className="absolute inset-x-0 bottom-0 mx-auto max-w-6xl px-6 pb-12 text-white">
          <div className="mb-4 flex flex-wrap gap-2 text-sm font-bold">
            ...
            <span className="rounded-full bg-white/20 px-4 py-2 backdrop-blur">산행 · 전망 · 자연</span>
          </div>
          <h1 className="text-5xl font-black tracking-tight md:text-7xl">삼각산</h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-white/90 md:text-xl">정상석이 자리한 해발 343m 대청도의 대표 산행 명소</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl space-y-8 px-6 pb-16">
        <div className="rounded-[2rem] bg-white p-8 shadow-sm md:p-10">
          ...
          <h2 className="mt-2 text-3xl font-black md:text-4xl">🌊 삼각산은 어떤 곳인가요?</h2>
          <p className="mt-6 text-[17px] leading-8 text-gray-700">정상석이 자리한 해발 343m 대청도의 대표 산행 명소.</p>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-[2rem] bg-white p-8 shadow-sm">
            ...
            <ul className="mt-5 space-y-4 leading-7 text-gray-700">
              <li>• 해발 343m 정상석</li><li>• 대청도 산 능선 풍경</li><li>• 정상에서 느끼는 섬 산행의 매력</li>
            </ul>
          </div>
          <div className="rounded-[2rem] bg-white p-8 shadow-sm">
            <h2 className="text-2xl font-black">🥾 방문 팁</h2>
            <ul className="mt-5 space-y-4 leading-7 text-gray-700">
              <li>• 산행 전 기상상황을 확인하세요.</li><li>• 운동화나 등산화를 권장합니다.</li><li>• 생수와 바람막이를 준비하세요.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="rounded-[2rem] bg-white p-8 shadow-sm md:p-10">
          ...
          <div className="mt-8 space-y-6 text-gray-700">
            <div><h3 className="text-lg font-bold">Q. 삼각산은 사진 찍기 좋은가요?</h3><p className="mt-2 leading-7">네. 대청도의 자연 풍경을 배경으로 여행 사진을 남기기 좋습니다. 날씨와 시간대에 따라 풍경이 달라집니다.</p></div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Link href="/place/miadong-beach" className="rounded-3xl bg-white p-6 font-extrabold shadow-sm hover:shadow-md">← 미아동해변</Link>
          <Link href="/place/maebawi-observatory" className="rounded-3xl bg-gray-900 p-6 text-right font-extrabold text-white shadow-sm hover:shadow-md">매바위전망대 →</Link>
        </div>
      </section>

      <PlaceReviews placeSlug="samgaksan" placeName="삼각산" />
    </main>
  );
}
